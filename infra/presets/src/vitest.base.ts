...
export default defineProject({
  test: {
    environment: 'node',
    // Tests live in a `test/` folder next to the `src/` they cover — at the workspace root for a
    // single-layer package, or under each layer folder in an aggregate one (`core/*/test`), so the
    // glob descends. See `@.claude/rules/testing.md`.
