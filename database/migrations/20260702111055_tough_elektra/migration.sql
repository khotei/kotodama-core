CREATE EXTENSION IF NOT EXISTS pg_trgm;--> statement-breakpoint
CREATE TYPE "async_job_status" AS ENUM('pending', 'running', 'succeeded', 'failed');--> statement-breakpoint
CREATE TYPE "language" AS ENUM('ru', 'en', 'es', 'fr', 'de', 'zh', 'ja', 'hi', 'ar', 'uk');--> statement-breakpoint
CREATE TYPE "word_job_stage" AS ENUM('fetch_source', 'enrich_etymology', 'enrich_tiers', 'enrich_authors', 'enrich_visuals', 'final_review');--> statement-breakpoint
CREATE TABLE "async-word-jobs" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	"word" text NOT NULL,
	"language" "language" DEFAULT 'en'::"language" NOT NULL,
	"stage" "word_job_stage" NOT NULL,
	"status" "async_job_status" DEFAULT 'pending'::"async_job_status" NOT NULL,
	"result" jsonb,
	"error" jsonb,
	"started_at" timestamp with time zone,
	"finished_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "async_word_jobs_stage_uq" UNIQUE("word","language","stage")
);
--> statement-breakpoint
CREATE TABLE "word_status_counts" (
	"language" "language" DEFAULT 'en'::"language",
	"status" "async_job_status",
	"n" integer DEFAULT 0 NOT NULL,
	CONSTRAINT "word_status_counts_pkey" PRIMARY KEY("language","status")
);
--> statement-breakpoint
CREATE TABLE "words" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	"word" text NOT NULL,
	"language" "language" DEFAULT 'en'::"language" NOT NULL,
	"status" "async_job_status" NOT NULL,
	"core_definition" text,
	"lexical" jsonb,
	"pronunciation" jsonb,
	"tiers" jsonb,
	"etymology" jsonb,
	"author_examples" jsonb,
	"cultural_guide" jsonb,
	"relations" jsonb,
	"translations" jsonb,
	"visuals" jsonb,
	"sources" jsonb,
	"source_versions" jsonb,
	"frequency" jsonb,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "words_word_language_unique" UNIQUE("word","language"),
	CONSTRAINT "words_succeeded_content_present" CHECK ("status" <> 'succeeded' OR ("core_definition" IS NOT NULL AND "lexical" IS NOT NULL AND "pronunciation" IS NOT NULL AND "tiers" IS NOT NULL AND "etymology" IS NOT NULL AND "author_examples" IS NOT NULL AND "cultural_guide" IS NOT NULL AND "relations" IS NOT NULL AND "translations" IS NOT NULL AND "visuals" IS NOT NULL AND "sources" IS NOT NULL AND "source_versions" IS NOT NULL))
);
--> statement-breakpoint
CREATE INDEX "words_language_created_at_word_idx" ON "words" ("language","created_at" DESC NULLS LAST,"word");--> statement-breakpoint
CREATE INDEX "words_language_status_created_at_word_idx" ON "words" ("language","status","created_at" DESC NULLS LAST,"word");--> statement-breakpoint
CREATE INDEX "words_language_pos_created_at_word_idx" ON "words" ("language",("lexical" ->> 'partOfSpeech'),"created_at" DESC NULLS LAST,"word") WHERE "lexical" IS NOT NULL;--> statement-breakpoint
CREATE INDEX "words_word_core_definition_trgm_idx" ON "words" USING gin ("word" gin_trgm_ops,"core_definition" gin_trgm_ops);--> statement-breakpoint
CREATE INDEX "words_language_word_prefix_idx" ON "words" ("language","word" text_pattern_ops);--> statement-breakpoint
-- Counter trigger (hand-authored — Drizzle can't express a trigger). Keeps `word_status_counts`
-- in lockstep with `words` per-status, in the SAME transaction as each write (a row-level AFTER
-- trigger is synchronous/IMMEDIATE), so an unfiltered count can never lag the list.
CREATE FUNCTION "words_status_counts_sync"() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
	IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
		INSERT INTO "word_status_counts" ("language", "status", "n")
			VALUES (NEW."language", NEW."status", 1)
			ON CONFLICT ("language", "status") DO UPDATE SET "n" = "word_status_counts"."n" + 1;
	END IF;
	IF (TG_OP = 'UPDATE' OR TG_OP = 'DELETE') THEN
		UPDATE "word_status_counts" SET "n" = "n" - 1
			WHERE "language" = OLD."language" AND "status" = OLD."status";
	END IF;
	RETURN NULL;
END;
$$;--> statement-breakpoint
CREATE TRIGGER "words_status_counts_sync_trg"
	AFTER INSERT OR UPDATE OF "status", "language" OR DELETE ON "words"
	FOR EACH ROW EXECUTE FUNCTION "words_status_counts_sync"();