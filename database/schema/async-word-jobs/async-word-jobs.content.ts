 ...
 * entity is fully typed rather than opaque `Json`. core derives its `AsyncWordJobModel` from the entity,
 * so every job shape has exactly one author here.
 */
 ...
 * is retained (optional) for backend debugging and never crosses into core's FE-facing error view
 * (`JobErrorView`), which surfaces only `message` + `type`.
 */
