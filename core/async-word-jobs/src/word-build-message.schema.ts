...
/**
 * The build-dispatch message — the only state crossing the queue from {@link WordBuildRequester} (enqueue)
...
/**
 * JSON-string codec for {@link WordBuildMessage}: `WordBuildRequester` encodes a message to the string
 * body `QueueService.send` takes, and the worker decodes a received body back through the same schema —
