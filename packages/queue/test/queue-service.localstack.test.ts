...
import { QueueService } from '../src'
import { drainQueue, QueueLocalStackLive } from '../src/testing'

// Contract test for the real adapter: `QueueServiceLive` over a per-file LocalStack SQS container.
// With the in-memory fake removed, ALL queue tests run on this real adapter (the queue analogue of
// the Testcontainers DB); these cases pin the adapter's own contract — a real round-trip and the
// received→invisible ack semantics the worker's delete-on-success rests on.
it.layer(QueueLocalStackLive, { timeout: '120 seconds' })((it) => {
  it.effect('QueueServiceLive round-trips send → receive → delete against LocalStack SQS', () =>
    Effect.gen(function* () {
      yield* drainQueue
