# packages/storage — `@kotodama/storage`

An object-storage port over `Bun.S3Client` (a Bun global, no npm dep), same base + bound-wrapper
split as `@kotodama/queue`: **`StorageClient`** (one client, `put(bucket, key, …)` takes the bucket
...
- **The deterministic key scheme lives only here** — `imageKey`/`authorKey` in `storage-types.ts`;
  ...
  imports nothing internal except `@kotodama/config`); `put` returns the same key so a stage threads
...
## Testing — `@kotodama/storage/testing`

Real `Bun.S3Client` over a per-file LocalStack S3 container (`StorageLocalStackLive`,
`SERVICES=s3`) — no in-memory fake; copies the `@kotodama/queue/testing` template (see
...
**May import:** `effect`, `@kotodama/config` (+ the `Bun` global).
