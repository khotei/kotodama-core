...
split as `@lexiai/queue`: **`StorageClient`** (one client, `put(bucket, key, …)` takes the bucket
...
- **The deterministic key scheme lives only here** — `imageKey`/`authorKey` in `storage-types.ts`;
  ...
  imports nothing internal except `@lexiai/config`); `put` returns the same key so a stage threads
...
`SERVICES=s3`) — no in-memory fake; copies the `@lexiai/queue/testing` template (see
...
**May import:** `effect`, `@lexiai/config` (+ the `Bun` global).
