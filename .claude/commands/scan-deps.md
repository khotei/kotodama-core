...
1. Run `bun run lint` — Biome's `style/noRestrictedImports` rules fail on any forbidden cross-layer import (e.g. `packages/*` importing `core/*`, or `database/` importing `repositories/*`).
