...
1. Run `bun run lint` — Biome's `style/noRestrictedImports` rules fail on any forbidden cross-layer import (e.g. `apps/web` importing `@lexiai/database`, or `packages/*` importing `core/*`).
2. Confirm no application code imports from `repos/**`:
   `grep -rn "repos/" apps core repositories database packages --include='*.ts' --include='*.tsx' || echo "none"`
