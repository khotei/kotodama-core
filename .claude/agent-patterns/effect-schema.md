...
- Use `effect/Schema` — **never Zod**. Word shapes are authored in `core/database/` (content schemas + `WordEntity`); consumers use them directly, and the consuming layers author only computed view/read models (`WordStateView`).
