...
| Effect | **Effect v4 (beta)** | `effect@beta` (pinned `4.0.0-beta.74` in `bun.lock`). Use `Context.Service`/`Context.Tag` (v4 renamed back from `ServiceMap`). In-beta APIs live under `effect/unstable/*`. |
