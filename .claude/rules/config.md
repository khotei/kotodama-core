...
through **`@lexiai/config`** — both the env-key registry and the env-file loader; nothing else reads
`.env`.
...
- **`ConfigProviderLive`** loads the **repo-root** `.env` as a **fallback under `process.env`**
  ...
  URL (`@lexiai/database/testing`), so there is no `.env.test` and nothing to protect the dev DB
