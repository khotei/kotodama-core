...
import { Language } from '@lexiai/core-words'
 ...
 * mirrors `WordFinder.find`). The build lifecycle is a separate concern read via {@link getWordState}. "Auto"
...
/**
 * The build-state read — the `(word, language)`'s {@link WordStateModel} (`succeeded` / `running` /
...
const getWordState = HttpApiEndpoint.get('getWordState', '/words/:language/:word/state', {
  params: { language: Language, word: Schema.String },
  success: Schema.NullOr(WordStateModel),
})
 ...
 * {@link WordStateModel} (the just-created resource). Every other state is a typed HTTP error:
